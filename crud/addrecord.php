...
<nav class="navbar navbar-expand-md">
    <!-- Brand -->
    <a class="navbar-brand" href="#">LOCALE LABEL</a>
...
<div class="container-fluid">
    ...
    <div class="row">
      <div class="col-md-4">
        <h3 class="text-center text-info text-dark mt-2">Add Record</h3>
        <form action="action.php" method="post" enctype="multipart/form-data">
          <input type="hidden" name="id" value="<?= $id; ?>">
          <div class="form-group">
           <!-- <input type="text" name="name" value="    = $name; ?>" class="form-control" placeholder="Enter name" required>-->
            <input type="text" name="product_name" value="<?= $product_name; ?>" class="form-control" placeholder="Enter name" required>
          </div>
          <div class="form-group">
           <!-- <input type="email" name="email" value="   = $email; ?>"class="form-control" placeholder="Enter e-mail" required>-->
            <input type="text" name="product_price" value="<?= $product_price; ?>" class="form-control" placeholder="Enter price" required>
          </div>
          <div class="form-group">
            <!-- <input type="tel" name="phone" value="    = $phone; ?>" class="form-control" placeholder="Enter phone" required>-->
            <input type="text" name="product_code" value="<?= $product_code; ?>" class="form-control" placeholder="Enter product code" required>
          </div>
          <div class="form-group">
            <input type="hidden" name="oldimage" value="<?= $product_image; ?>">
            <input type="file" name="product_image" class="custom-file">
      ...
      <div class="col-md-8">
        <?php
          $query = 'SELECT * FROM charlottefolk_product';
          $stmt = $conn->prepare($query);
          $stmt->execute();
          $result = $stmt->get_result();
        ?>
        <h3 class="text-center text-info  text-dark mt-2">LOCALE LABEL RECORDS</h3>
        <h3 class="text-center text-dark mt-2">Charlotte Folk</h3>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                <a href="addrecord.php?edit=<?= $row['id']; ?>" class="badge badge-success p-2">Edit</a>
              </td>
            </tr>
            <?php } ?>
          </tbody>
